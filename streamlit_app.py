...
# ==========================================
# CUSTOM CSS FOR BACKGROUND (ADD THIS SECTION)
# ==========================================
def add_background():
    """เพิ่ม CSS และ HTML เพื่อแทรกภาพพื้นหลังจางๆ"""
    st.markdown(
        f"""
        <style>
        .stApp {{
            background-color: rgba(255, 255, 255, 0.85); /* พื้นหลังขาวจางๆ เพื่อให้อ่านง่าย */
            background-image: url("https://images.pexels.com/photos/1519656/pexels-photo-1519656.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1"); /* ตัวอย่างรูปฟาร์มไก่ */
            background-size: cover;
            background-position: center;
            background-repeat: no-repeat;
            background-attachment: fixed;
            opacity: 1; /* ความโปร่งใสของพื้นหลังโดยรวม */
        }}
        /* ลดความทึบของกล่องข้อมูลเพื่อให้พื้นหลังจางๆ ปรากฏ */
        [data-testid="stMetricValue"], [data-testid="stExpander"], [data-testid="stSelectbox"] {{
            opacity: 0.95;
        }}
        /* ทำให้แผนภูมิมีพื้นหลังโปร่งใส */
...
add_background()  # เรียกใช้งานฟังก์ชันพื้นหลัง

# ==========================================
# REST OF THE CODE (UNCHANGED, EXCEPT OPTIMIZATIONS)
# ==========================================
...
def load_ingredients_from_supabase():
    try:
        ...
        if not df.empty:
            # ล้างช่องว่างในชื่อวัตถุดิบป้องกันปัญหาคีย์ใน PuLP
...
if st.button("🚀 ประมวลผลและคำนวณสารอาหารที่แม่นยำที่สุด", use_container_width=True, type="primary"):
    if df_ingredients is not None and not df_ingredients.empty:
        ...
        if pulp.LpStatus[prob.status] == "Optimal":
            ...
            st.error("❌ ไม่สามารถคำนวณตามเงื่อนไขสารอาหารมาตรฐานได้ คลังวัตถุดิบในระบบปัจจุบันอาจมีสารอาหารไม่เพียงพอตามเกณฑ์ขั้นต่ำ")
    else:
        st.error("❌ ไม่สามารถดึงคลังวัตถุดิบมาจากฐานข้อมูลได้")
...
if st.session_state.calculated and st.session_state.df_result is not None:
    ...
    with report_left:
        ...
        fig.update_layout(
            ...
            paper_bgcolor='rgba(255, 255, 255, 0)',  # ทำให้แผนภูมิโปร่งใส
            plot_bgcolor='rgba(255, 255, 255, 0)'
        )
        st.plotly_chart(fig, use_container_width=True)
        
        # Real-time Nutrient Accuracy Indicators
