...
# ==========================================
def add_background():
    """ฟังก์ชัน CSS จัดการพื้นหลังภาพไข่ไก่แบบโปร่งแสง และคุมโทนกล่อง UI ให้สแกนสายตาง่าย"""
    st.markdown(
        """
        <style>
        /* ล้างสีพื้นหลังโครงสร้างหลักของ Streamlit */
        .stApp, [data-testid="stAppViewContainer"], [data-testid="stHeader"] {
            background-color: transparent !important;
        }
        
        /* ใส่ภาพพื้นหลังมินิมอลแบบ Fixed ไม่เลื่อนตามสกรอลล์ */
        .stApp::before {
            content: "";
            position: fixed;
        ...
        /* ปรับแต่งการ์ดคอลัมน์ (Grid Columns) สไตล์ Glassmorphism เข้ม */
        ...
        /* กำหนดสีตัวอักษรของ Label และหัวข้อของ Widgets ทั้งหมด */
        label, [data-testid="stWidgetLabel"] p {
            color: #ffffff !important;
            font-weight: 500 !important;
        }
        
        /* ออกแบบกล่องแสดงผล Metric */
...
try:
    ...
    st.error("❌ ไม่พบข้อมูลการเชื่อมต่อ Supabase ใน Streamlit Secrets (กรุณาตรวจไฟล์ secrets.toml)")

@st.cache_data(ttl=5)
def load_ingredients_from_supabase():
    """ดึงข้อมูลวัตถุดิบและแปลงฟอร์แมตเป็นภาษาไทย (ภาษาอังกฤษ)"""
    if supabase is None: return None
    try:
        response = supabase.table("ingredients").select("id, category, name_th, name_en").execute()
        df = pd.DataFrame(response.data)
        if not df.empty:
            # 💡 จัดฟอร์แมตวัตถุดิบ: "ชื่อไทย (ชื่ออังกฤษ)"
            df['name'] = df['name_th'].str.strip() + " (" + df['name_en'].str.strip() + ")"
            
            # จำลองค่าสารอาหารและราคาชั่วคราว (Fallback) เนื่องจากตารางหลักยังไม่มีคอลลัมน์เหล่านี้
            ...
            if 'lysine_pct' not in df.columns: df['lysine_pct'] = 1.2
            if 'methionine_pct' not in df.columns: df['methionine_pct'] = 0.5
            if 'max_limit_pct' not in df.columns: df['max_limit_pct'] = 100.0
        return df
    except Exception:
        return None
...
# จัดเตรียมรายชื่อกลุ่มเพื่อป้อนเข้า Selectbox แรก
...
with input_col1:
    ...
    # 2. กรองสายพันธุ์ให้สัมพันธ์กัน และแสดงชื่อรูปแบบ "ภาษาไทย (ภาษาอังกฤษ)"
    if not df_breeds_raw.empty:
        filtered_breeds = sorted(df_breeds_raw[df_breeds_raw['category'] == selected_group]['display_name'].dropna().unique().tolist())
    else:
        # Fallback ในกรณีที่ยังไม่เคลียร์ RLS บนหน้าเว็บ Supabase
        fallback_dict = {
            "สายพันธุ์เชิงพาณิชย์": ["ไฮไลน์ บราวน์ (Hy-Line Brown)", "ไฮไลน์ ดับเบิลยู-36 (Hy-Line W-36)", "โลห์มันน์ บราวน์ (Lohmann Brown)", "ไอเอสเอ บราวน์ (ISA Brown)"],
    ...
    # กล่องเลือกสายพันธุ์และระยะการเลี้ยง
...
# ปุ่มเริ่มคำนวณอาหารเชิงเส้น (Linear Programming Optimization)
if st.button("🚀 ประมวลผลและคำนวณสารอาหารที่แม่นยำที่สุด", use_container_width=True, type="primary"):
    if df_ingredients is not None and not df_ingredients.empty:
        # กำหนดเกณฑ์โภชนาการ (Nutrient Constraints)
        AUTO_PROTEIN = 20.0
        AUTO_ME = 2900.0
        AUTO_LYSINE = 1.10
        ...
        if pulp.LpStatus[prob.status] == "Optimal":
            ...
            for _, row in df_ingredients.iterrows():
                ...
                if w and w > 0.01: # แสดงผลเฉพาะวัตถุดิบที่มีการใช้งานจริงมากกว่า 0.01%
                    result_list.append({
        ...
        st.error("❌ ไม่สามารถดึงคลังวัตถุดิบหลักมาจากฐานข้อมูล Supabase ได้")

st.markdown("---")

# ==========================================
# SECTION 2: รายงานผลลัพธ์และการวิเคราะห์ประสิทธิภาพ
# ==========================================
...
if st.session_state.calculated and st.session_state.df_result is not None:
    # ฟังก์ชันคำนวณคาดการณ์ด้านการเงิน
    ...
    with report_left:
        # 2. กราฟวงกลมแสดงสัดส่วนวัตถุดิบอาหาร
        ...
        # 3. แถบความคืบหน้าตรวจสอบความเข้มข้นสารอาหารจริงที่ได้
    ...
    with report_right:
        # 4. ตารางสรุปสูตรผสมอาหารจริงสำหรับฟาร์ม
    ...
    st.info("💡 **ระบบพร้อมใช้งาน:** ตั้งค่ากลุ่มไก่และสายพันธุ์ที่ดึงตรงจากฐานข้อมูลด้านบน จากนั้นกดปุ่มประมวลผลสูตรอาหารได้ทันทีครับ")
