...
# ดึงข้อมูลจาก Streamlit Secrets สำหรับเชื่อมต่อ Supabase ผ่าน HTTP API
...
def load_ingredients_from_supabase():
    try:
        # ดึงสารอาหารทั้งหมดรวมถึง ตัวที่เพิ่งเพิ่มใหม่ (ME, Lysine, Methionine) ผ่าน API
        response = supabase.table("ingredients").select("name, price_per_kg, protein_pct, fat_pct, me_kcal_per_kg, lysine_pct, methionine_pct, max_limit_pct").execute()
...
with tabs[0]: # ทำงานในแท็บ "คำนวณสูตรอาหาร"
    ...
    with col_left:
        ...
        if st.button("🚀 ประมวลผลสูตรอาหาร", use_container_width=True, type="primary"):
            if df_ingredients is not None and not df_ingredients.empty:
                ...
                st.error("ไม่พบข้อมูลวัตถุดิบในฐานข้อมูล หรือตารางว่างเปล่า")
    ...
    with col_right:
        ...
        if st.session_state.calculated and st.session_state.df_result is not None:
            # 🍩 ใช้ Plotly Express วาดกราฟโดนัทสวยงามและรองรับภาษาไทย 100%
            fig = px.pie(
