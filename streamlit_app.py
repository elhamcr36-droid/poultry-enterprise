...
# 🔄 ปรับให้ชื่อภาษาไทยขึ้นก่อน และตามด้วยอังกฤษในวงเล็บ
...
# 🔄 ปรับชื่อสายพันธุ์ทั้งหมดเป็น: ภาษาไทย (ภาษาอังกฤษ) ตามรูปแบบคอมโบ้บ็อกซ์ในรูปภาพของคุณ
BREED_PROFILES = {
...
# ==========================================
st.markdown("### 📈 6. ระบบบันทึกประสิทธิภาพผลผลิต (บันทึกขึ้นฐานข้อมูล Supabase Cloud)")
...
with track_col1:
    with st.form("supabase_sync_form"):
        in_date = st.text_input("วันที่บันทึกสถิติฟาร์ม:", value="04/06")
        f_name = st.text_input("ชื่อสูตรอาหารที่ใช้ในรอบนี้:", value="สูตร AI แนะนำ")
        lay_r = st.number_input("อัตราการไข่ที่เก็บได้วันนี้ (%):", value=86.2)
        crack_r = st.number_input("อัตราไข่บุบหรือแตกหน้ากรงวันนี้ (%):", value=1.5)
            ...
            if "your-project" not in SUPABASE_URL and SUPABASE_KEY != "your-anon-key":
                try:
                    ...
                    if response.status_code in [200, 201]:
                        st.success(f"☁️ ระบบเชื่อมต่อสำเร็จ: ส่งข้อมูลและสถิติไปยังระบบฐานข้อมูลคลาวด์ Supabase เรียบร้อยแล้ว!")
                    else:
                        st.error(f"❌ เกิดข้อผิดพลาดจากเซิฟเวอร์ Supabase (รหัสสถานะ: {response.status_code}): {response.text}")
                except Exception as e:
                    st.error(f"❌ ไม่สามารถติดต่อคลาวด์เซิฟเวอร์ได้เนื่องจากปัญหาเครือข่าย: {e}")
            else:
                st.info("⚠️ ระบบทำงานในโหมด Offline Local: บันทึกข้อมูลลงในฐานข้อมูลจำลองชั่วคราวแล้ว (ข้อมูลจะส่งขึ้นคลาวด์จริงเมื่อมีการตั้งค่า API Key ที่แถบเมนูด้านซ้าย)")
            
            st.rerun()

with track_col2:
    fig = px.line(st.session_state.tracker_data, x="วันที่", y=["อัตราการไข่ (%)", "อัตราไข่บุบแตก (%)"], markers=True, title="กราฟวิเคราะห์ความสัมพันธ์ของผลผลิตหลังปรับเปลี่ยนสูตรอาหาร")
