...
def load_ingredients_from_supabase():
    ...
    try:
        ...
        if not df.empty:
            ...
            # จำลองค่าสารอาหารและราคาชั่วคราวเนื่องจากตารางหลักยังไม่มีคอลัมน์เหล่านี้
...
def load_chicken_breeds_dataframe():
    """ดึงข้อมูลสายพันธุ์จากตาราง chicken_breeds ทั้งหมดเพื่อนำมาใช้งาน"""
    if supabase is None: return pd.DataFrame()
    try:
        response = supabase.table("chicken_breeds").select("category, name_th, name_en").execute()
        df = pd.DataFrame(response.data)
        if not df.empty:
            # จัดฟอร์แมตสายพันธุ์ไก่สำหรับแสดงผล: "ชื่อไทย (ชื่ออังกฤษ)"
...
# 🛠️ ดึงชื่อกลุ่มภาษาไทยตรงๆ จากฐานข้อมูล เพื่อนำมาจับคู่เงื่อนไขให้ถูกต้องแม่นยำ
...
with input_col1:
    ...
    # 1. เลือกกลุ่มไก่ไข่ (แสดงผลภาษาไทยเพื่อให้ตรงคอลัมน์ category ในฐานข้อมูล)
    selected_group = st.selectbox("กลุ่มไก่ไข่", list_groups, index=0, on_change=reset_calculation)
    
    # 2. กรองข้อมูลจาก Dataframe ตรงๆ เมื่อตัวเลือกกลุ่มแมตช์กัน ข้อมูลสายพันธุ์จากฐานข้อมูลจริงจะไหลมาครบถ้วน
    if not df_breeds_raw.empty:
        filtered_breeds = sorted(df_breeds_raw[df_breeds_raw['category'] == selected_group]['display_name'].dropna().unique().tolist())
    else:
        # Fallback สํารองกรณีดึงข้อมูลไม่สำเร็จ
        fallback_dict = {
...
# ปุ่มเริ่มคำนวณอาหารเชิงเส้น (Linear Programming)
