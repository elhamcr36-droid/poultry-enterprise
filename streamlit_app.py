...
def load_ingredients_from_supabase():
    ...
    try:
        ...
        if not df.empty:
            ...
            # จำลองค่าสารอาหารเพื่อใช้ในการคำนวณชั่วคราว
...
# 🛠️ ฟังก์ชันดึงโครงสร้างสายพันธุ์แบบ Dataframe เพื่อนำมาผูกความสัมพันธ์กลุ่ม-สายพันธุ์
@st.cache_data(ttl=5)
def load_chicken_breeds_dataframe():
    """ดึงข้อมูลดิบจากตารางสายพันธุ์ไก่ทั้งหมด"""
    if supabase is None: return pd.DataFrame()
    try:
        response = supabase.table("chicken_breeds").select("category, name_th").execute()
...
# กำหนดตัวเลือกเริ่มต้นในกรณีที่ Supabase RLS ยังไม่ได้เปิดใช้งาน
...
with input_col1:
    ...
    # 🛠️ 2. กรองรายชื่อสายพันธุ์สัมพันธ์ตามกลุ่มที่เลือกในแบบไดนามิกแบบเรียลไทม์
    if not df_breeds_raw.empty:
        filtered_breeds = sorted(df_breeds_raw[df_breeds_raw['category'] == selected_group]['name_th'].dropna().unique().tolist())
    else:
        # Fallback เผื่อ RLS ยังไม่ทำงาน
        fallback_dict = {
            "สายพันธุ์เชิงพาณิชย์": ["ไฮไลน์ บราวน์", "ไฮไลน์ ดับเบิลยู-36", "โลห์มันน์ บราวน์", "ไอเอสเอ บราวน์"],
            "สายพันธุ์แท้": ["เลกฮอร์นขาว", "โรดไอแลนด์เรด"],
            "กลุ่มไข่สีพิเศษ": ["อาราอูคานา", "อเมราอูคานา"],
            "สายพันธุ์พื้นเมืองและพรีเมียม": ["ซิลกี้ หรือไก่ไหม", "มาร็องส์"]
        }
        filtered_breeds = fallback_dict.get(selected_group, ["ไอเอสเอ บราวน์"])
        
    # แสดงกล่องเลือกสายพันธุ์ที่ถูกปลดล็อกและสัมพันธ์กันแล้ว
...
if st.button("🚀 ประมวลผลและคำนวณสารอาหารที่แม่นยำที่สุด", use_container_width=True, type="primary"):
    if df_ingredients is not None and not df_ingredients.empty:
        ...
        if pulp.LpStatus[prob.status] == "Optimal":
            ...
            st.success("🎉 ล็อกสัดส่วนและสูตรอาหารที่คุ้มค่าที่สุดเสร็จสิ้น!")
        else:
            st.error("❌ ไม่สามารถคำนวณตามเงื่อนไขสารอาหารได้")
    else:
        st.error("❌ ไม่สามารถดึงคลังวัตถุดิบได้")
...
if st.session_state.calculated and st.session_state.df_result is not None:
    ...
    with report_left:
        ...
        with prog_col2:
            st.progress(min(st.session_state.calculated_me / 2900.0, 1.0), text=f"พลังงาน: {st.session_state.calculated_me:.0f} kcal")
            st.progress(min(st.session_state.calculated_methionine / 0.45, 1.0), text=f"เมทไธโอนีน: {st.session_state.calculated_methionine:.2f}%")
